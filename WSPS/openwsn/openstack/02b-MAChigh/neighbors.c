                          ...
                          bool		   isLowerPowerRunning) 
{
   ...
   for (i=0;i<MAXNUMNEIGHBORS;i++) 
   {
      if (isThisRowMatching(l2_src,i)) 
      {
		 ...
		 neighbors_vars.neighbors[i].LOWERPOWER_RUN=isLowerPowerRunning;
   ...
   if (newNeighbor==TRUE) 
   {//lcg 20180604 l2_src.type:2;addr64b[0]:0x15,rssi:0xee,joinPrioPresent:1;joinprio:0,insecure:1
      registerNewNeighbor(l2_src, rssi, asnTs, joinPrioPresent, joinPrio, insecure,isLowerPowerRunning);
	  neighbors_setNeighborRank(0,256);//lcg 20180907 add set the first eb as parrent
	  icmpv6rpl_updateMyDAGrankAndParentSelection();//lcg 20180907 add
   }
}
...
/*added by zyx,check if there is any lowerpower run mote,*/
bool AnyNeighborISlowerPowerRun(void)
{
	uint8_t i;
	for(i=0;i<MAXNUMNEIGHBORS;i++)
	{
		if(neighbors_vars.neighbors[i].used==TRUE&&neighbors_vars.neighbors[i].LOWERPOWER_RUN==TRUE)
		{
			return TRUE;
		}
	}
...
/*use flash canshu faunhi congjishuliang*/
uint8_t getSlavenums(void)
{
	/*uint8_t slavenumbers;
	slavenumbers=*(__IO uint8_t *)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_NUMBEROFMOTES)-1;
	if(slavenumbers<=3)
	{
		return slavenumbers;
	}else
...
uint8_t getSlaveMAC(uint8_t ADDR)
{
	return neighbors_vars.neighbors[(ADDR)&0x03].addr_64b.addr_16b[0];
}
uint8_t getSlaveLOWERPOWERMODE(uint8_t ADDR)
{
	return neighbors_vars.neighbors[(ADDR)&0x03].LOWERPOWER_RUN;
}
uint8_t getSlaveRSSI(uint8_t ADDR)
{
	return neighbors_vars.neighbors[(ADDR)&0x03].rssi;
}