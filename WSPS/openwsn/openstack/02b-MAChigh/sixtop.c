...
#define MAINTENANCE_PERIOD        8//30

//=========================== variables =======================================

sixtop_vars_t sixtop_vars;
sixtop_management_calling_vars_t sixtop_ManCall_vars;//zyx added
...
void sixtop_management_Calling(bool Callingon);

//=========================== public ==========================================

void sixtop_init() {
    memset(&sixtop_ManCall_vars,0,sizeof(sixtop_management_calling_vars_t));//zyx
    //sixtop_management_Calling(OFF);
    sixtop_ManCall_vars.CallingCounter=0;
    sixtop_ManCall_vars.CallingisBUSY=0;
    sixtop_ManCall_vars.CallingSTOPCNT=FIRSTCallingONPeriod_S;
    //sixtop_vars.periodMaintenance  = 872 +(openrandom_get16b()&0xff);//zyx20181203
    sixtop_vars.periodMaintenance  = 1000;//872;unit ms
    ...
    // lcg 20180426  pri: sixtop_vars.ebPeriod           = EB_PORTION*(neighbors_getNumNeighbors()+1);
    sixtop_vars.ebPeriod           = EB_PORTION;
...
void task_sixtopNotifReceive() 
{
    ...
    // update neighbor statistics updata the neighbors
    neighbors_indicateRx(
        ...
        msg->LOWERPOWER_RUN //this mode is added by zyx
    );//
    if(idmanager_getIsDAGroot()&&neighbors_getNumNeighbors()==getSlavenums())//if I am master and the neighbors I get is the number I set I will quit the calling state
    {
    	sixtop_management_Calling(OFF);
    ...
    lenIE=0;
    if(
        (msg->l2_frameType              == IEEE154_TYPE_DATA|| msg->l2_frameType== IEEE154_TYPE_CALLING)//zyx added
        &&
	...
	//I am here zyx20181206
	
    // send the packet up the stack, if it qualifies
    switch (msg->l2_frameType) {
    /*I can prepare calling answer packet here zyx20180903*/
    ...
    case IEEE154_TYPE_CMD:
        if (msg->length>0) 
        {
            ...
            iphc_receive(msg);//I am here zyx20181206
        } else {
            // free up the RAM
            openqueue_freePacketBuffer(msg);
        }
        break;
    case IEEE154_TYPE_CALLING:
		sixtop_sendSepecialKA();
    	break;
...
void sixtop_sendingEb_timer_cb(opentimers_id_t id){
	...
	sixtop_vars.periodMaintenance  = 730 +(openrandom_get16b()&0x3f)*11;//lcg 20180525 6 frame +-2
    opentimers_scheduleIn(
        sixtop_vars.ebSendingTimerId,
        sixtop_vars.periodMaintenance,
        TIME_MS,
...
void timer_sixtop_sendEb_fired(){
    ...
    uint16_t newPeriod;
    // current period 
    // lcg 20180426  pri: newPeriod = EB_PORTION*(neighbors_getNumNeighbors()+1);
    newPeriod = EB_PORTION;
    if (
...
void timer_sixtop_management_fired(void) 
{
    static uint8_t Calling_Time=0;
    //sixtop_vars.mgtTaskCounter = (sixtop_vars.mgtTaskCounter+1)%MAINTENANCE_PERIOD;
	if(sixtop_vars.mgtTaskCounter%(MAINTENANCE_PERIOD)==0)
	{
		if(idmanager_getIsDAGroot()&&*(__IO uint8_t*)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_LOWERPOWER_CALLINGON)==1)//I am master 0 means no calling 1 means calling
		{
			neighbors_removeOld();
			if(neighbors_getNumNeighbors()<MaxSlaveNum)
			{
				
					sixtop_management_Calling(ON);
			}else
			{
				sixtop_management_Calling(OFF);
			}
	...
	sixtop_vars.mgtTaskCounter++;//zyx changed
}
#endif//back up
void timer_sixtop_management_fired(void) 
{
    //sixtop_vars.mgtTaskCounter = (sixtop_vars.mgtTaskCounter+1)%MAINTENANCE_PERIOD;
    static uint8_t My_neighbors_Num;

	if(sixtop_vars.mgtTaskCounter%(MAINTENANCE_PERIOD)==0)
	{
		neighbors_removeOld();
		if(idmanager_getIsDAGroot())//I am master 0 means no calling 1 means calling
		{
			if(*(__IO uint8_t*)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_LOWERPOWER_CALLINGON)==1)
			{
				My_neighbors_Num=neighbors_getNumNeighbors();
				if(My_neighbors_Num<MaxSlaveNum)
				{
					sixtop_management_Calling(ON);
				}else
				{
	...
	else
	{
		...
		if(idmanager_getIsDAGroot()&&*(__IO uint8_t*)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_LOWERPOWER_CALLINGON)==1)
		{
			My_neighbors_Num=neighbors_getNumNeighbors();
			if(My_neighbors_Num<MaxSlaveNum)
			{
	...
	sixtop_vars.mgtTaskCounter = (sixtop_vars.mgtTaskCounter+1)%MAINTENANCE_PERIOD;//zyx changed
...
port_INLINE void sixtop_sendEB() {
    ...
    eb = openqueue_getLastFreePacketBuffer(COMPONENT_SIXTOP);//zyx20181218
    if (eb==NULL) 
    {
        openserial_printError(
            COMPONENT_SIXTOP,
    ...
    eb->l2_frameType                     =IEEE154_TYPE_BEACON;//add framType ACK or NOACK
...
port_INLINE void sixtop_sendSepecialKA()
{
    ...
    if (sixtop_vars.busySendingKA==TRUE||openqueue_sixtopGetCoapSentPacket()!=NULL) 
    {
        // don't proceed if I'm still sending a KA,OR there is COAP packet in the queue I will not send a KA 
        return;
    }
 
	//kaNeighAddr =neighbors_getKANeighbor(0);
	kaNeighAddr=idmanager_getMyID(ADDR_PARENTID);
	kaPkt = openqueue_getLastFreePacketBuffer(COMPONENT_SIXTOP);
    ...
    //memcpy(&(kaPkt->l2_nextORpreviousHop), &idmanager_vars.myParentID.addr_64b[0],sizeof(open_addr_t));
   	memcpy(&(kaPkt->l2_nextORpreviousHop),kaNeighAddr,sizeof(open_addr_t));
   	///memcpy(&(kaPkt->l2_nextORpreviousHop),(open_addr_t *)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_MYPARENT_MAC_ADDRESS),sizeof(open_addr_t));
    // set l2-security attributes
...
port_INLINE void sixtop_sendKA() {
    ...
    if(openqueue_sixtopGetCoapSentPacket()!=NULL)
    {
    	return;//if I am sending coap packet,I will not send KA
    }
...
/* added by zyx calling by the mode 20181112 I will call the */
# if 0
void sixtop_management_Calling(bool Callingon)
{
	/**/
	if(Callingon)
	{
		sixtop_ManCall_vars.CallingisBUSY=TRUE;
	}
	else
	{
		/*quit	calling	mode*/
		ieee154e_changemode(HIGHPOWER_CALLINGOFF);
		sixtop_ManCall_vars.CallingisBUSY=FALSE;
		sixtop_ManCall_vars.CallingCounter=0;
		sixtop_ManCall_vars.CallingSTOPCNT=FIRSTCallingONPeriod_S;
		return;
	}
	if(sixtop_ManCall_vars.CallingisBUSY)
	{
		sixtop_ManCall_vars.CallingCounter++;
		if(sixtop_ManCall_vars.CallingCounter==1)
		{
			ieee154e_changemode(LOWERPOWER_CALLINGON);//call 3s
		}
		else if(sixtop_ManCall_vars.CallingCounter==CallingONTime_S)
		{
			ieee154e_changemode(HIGHPOWER_CALLINGOFF);
		}
		else if(sixtop_ManCall_vars.CallingCounter==sixtop_ManCall_vars.CallingSTOPCNT)
		{
			sixtop_ManCall_vars.CallingCounter=0;
			if(sixtop_ManCall_vars.FORSE_CALLINGON||*(__IO uint8_t*)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_MASTERFORSE_CALLINGON)==0x1C)	
			{
				sixtop_ManCall_vars.CallingSTOPCNT=(sixtop_ManCall_vars.CallingSTOPCNT<<1)+1;
...
/*                                                                                                            */
/*0                sixtop_ManCall_vars.CallingSTOPCNT-2                     sixtop_ManCall_vars.CallingSTOPCNT*/
/*no calling                                                calling                                           */
void sixtop_management_Calling(bool Callingon)
{
	/**/
	...
	{
		sixtop_ManCall_vars.CallingCounter++;
		if(sixtop_ManCall_vars.CallingCounter==sixtop_ManCall_vars.CallingSTOPCNT-2)
		{
			ieee154e_changemode(LOWERPOWER_CALLINGON);//call 3s
		}
		else if(sixtop_ManCall_vars.CallingCounter==sixtop_ManCall_vars.CallingSTOPCNT)
		{
			ieee154e_changemode(HIGHPOWER_CALLINGOFF);
			sixtop_ManCall_vars.CallingCounter=0;
			if(sixtop_ManCall_vars.FORSE_CALLINGON||*(__IO uint8_t*)(ADDR_PARAMENT_STARTADDRESS+OFFSETADDR_MASTERFORSE_CALLINGON)==0x01)	
			{
				return;
			}
			else
			{
				sixtop_ManCall_vars.CallingSTOPCNT=((sixtop_ManCall_vars.CallingSTOPCNT<<1)+1)&0xFF;
			}
		}
...
void sixtop_change_calling_mode(bool forse_callingon)
{
	sixtop_ManCall_vars.FORSE_CALLINGON=forse_callingon;
	sixtop_ManCall_vars.CallingSTOPCNT=FIRSTCallingONPeriod_S;
	sixtop_ManCall_vars.CallingCounter=0;
}
