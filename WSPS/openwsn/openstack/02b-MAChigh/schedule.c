...
void schedule_startDAGroot() {
   ...
   for (running_slotOffset=start_slotOffset;running_slotOffset<start_slotOffset+SCHEDULE_MINIMAL_6TISCH_ACTIVE_CELLS;running_slotOffset++) {
	   if(running_slotOffset==9)//start_slotOffset)//lcg  20180418//zyx20180726最后一个SLOT，用于把最后一个SLOT改为下行，其余SLOT改为上行
	   {
		  schedule_addActiveSlot(
			 running_slotOffset,                 // slot offset
...
\param shared           Whether this cell is shared (TRUE) or not (FALSE).  //lcg 20180531 shared for backoff only.
