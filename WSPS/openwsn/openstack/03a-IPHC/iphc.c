...
owerror_t iphc_sendFromForwarding(
    ...
    // error checking
    if (idmanager_getIsDAGroot()==TRUE &&0&& //lcg 20180622 add 0,bypass
...
void iphc_receive(OpenQueueEntry_t* msg) {
    ...
    if (
        ...
        packetfunctions_isBroadcastMulticast(&(ipv6_inner_header.dest))||1  //lcg 20180622 add ||1,bypass this condition.
    ) {
    		...
    		case CRITICAL_6LORH :
    			...
    			if(lorh_type<=RH3_6LOTH_TYPE_4){
    				...
    				if (lorh_type==5){
    					...
    					//lcg 20180611 dead here,run some days.print more then 20 one second. ;task pin get up and never down.
    					//lcg 20180614  add bellow. if in here,this "while"will never stop.
    					//lcg 20180619  add return. if in here,this "while"will never stop.
						temp_8b = *(uint8_t*)((msg->payload) + \
    					    					    *page_length + \
    					    					    extention_header_length +\
    					    						ipv6_outer_header->header_length);//20181015 Í¬²½
    					//log wrong inf
						openserial_printError(
								COMPONENT_IPHC,
								ERR_6LOWPAN_UNSUPPORTED,
...
uint8_t iphc_retrieveIPv6HopByHopHeader(
   ...
   if (
	   ...
	   //lcg 20180611 dead here,run some days.print more then 20 one second. ;task pin get up and never down.
	   // 20180614,the word up,judg wrong,donot come here.
       openserial_printError(
          COMPONENT_IPHC,
          ERR_6LOWPAN_UNSUPPORTED,
          (errorparameter_t)14,
