...
#define USER_SERIAL_INPUT_BUFFER_SIZE	1024
#define USER_SERIAL_OUTPUT_BUFFER_SIZE	1024
/**
...
*/
#define SERIAL_INPUT_BUFFER_SIZE  1024//200//zyxchanged20190215
#define INPUT_BUFFER_MASK       0x3FF
...
// frames sent mote->PC
#define SERFRAME_MOTE2PC_DATA                    ((uint8_t)'D')  // 0x44  lcg 20180528
#define SERFRAME_MOTE2PC_STATUS                  ((uint8_t)'S')// 0x53
#define SERFRAME_MOTE2PC_INFO                    ((uint8_t)'I')// 0x49
#define SERFRAME_MOTE2PC_ERROR                   ((uint8_t)'E')// 0x45
#define SERFRAME_MOTE2PC_CRITICAL                ((uint8_t)'C')// 0x43
#define SERFRAME_MOTE2PC_REQUEST                 ((uint8_t)'R')// 0x52
#define SERFRAME_MOTE2PC_SNIFFED_PACKET          ((uint8_t)'P')// 0x50

// frames sent PC->mote
#define SERFRAME_PC2MOTE_SETROOT                 ((uint8_t)'R')
#define SERFRAME_PC2MOTE_RESET                   ((uint8_t)'Q')// 0x51
...
#define SERFRAME_PC2MOTE_TRIGGERUSERIALBRIDGE    ((uint8_t)'B')// 0x42
...
typedef struct {
    ...
    uint8_t             inputBuf[SERIAL_INPUT_BUFFER_SIZE];//input buffer
    uint16_t			inputBufIdxW;//zyx added20190215
    uint16_t			inputBufIdxR;//zyx added20190215
    // output
    bool                outputBufFilled;
    uint16_t            outputCrc;
    uint16_t            outputBufIdxW;
    uint16_t            outputBufIdxR;
    uint8_t             outputBuf[SERIAL_OUTPUT_BUFFER_SIZE];//output debug information	
} openserial_vars_t;

typedef struct
{
	uint8_t inputBuf[USER_SERIAL_INPUT_BUFFER_SIZE];
	uint16_t inputBufIdxW;
	uint16_t inputBufIdxR;
	uint8_t outputBuf[USER_SERIAL_OUTPUT_BUFFER_SIZE];
	uint16_t outputBufIdxW;
	uint16_t outputBufIdxR;
}userserial_vars_t;
// admin
...
void Debug_UartOUTPUT_DMA(uint8_t *data,uint8_t len);//added by zyx
