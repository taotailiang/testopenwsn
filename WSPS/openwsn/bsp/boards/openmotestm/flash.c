...
void flash_init(void) {   
    //FLASH_Unlock();//zyx 
    //FLASH_ClearFlag(FLASH_FLAG_BSY | FLASH_FLAG_EOP|FLASH_FLAG_PGERR |FLASH_FLAG_WRPRTERR);//zyx
}

uint8_t flash_erasePage(uint32_t address) {
    uint8_t status;
    //status = FLASH_ErasePage(address);//zyx
    return status;
}

uint8_t  flash_erase_optByte(void) {
    uint8_t status;
    //status = FLASH_EraseOptionBytes();//zyx
    return status;
}

uint8_t flash_write(uint32_t address,uint16_t data) {
    uint8_t status;
    //status = FLASH_ProgramHalfWord(address,data);//zyx
    return status;
}

uint8_t flash_write_optByte(uint32_t address,uint8_t data){
    uint8_t status;
    //status = FLASH_ProgramOptionByteData(address,data);//zyx
    return status;
}

uint16_t flash_read(uint32_t address){
    uint16_t temp = 0x00;
    //temp = *(uint32_t*)address;//zyx
    return temp;
}
