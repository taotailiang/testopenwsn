...
#define MAXNUMNEIGHBORS  4//10//zyx20181214
...
enum {
   ...
   ADDR_PREFIX                         = 5,
   ADDR_ANYCAST                        = 6,
   ADDR_CALLING						   = 7,//added by zyx for the calling mode
   ADDR_PARENTID					   =8,//added by zyx 20181120
};
...
enum {
   ...
   WKP_UDP_TEST                =    0xf0b0,//lcg 20180621 for compress
};
...
enum {
   ...
   //user defined 20180713zyx
   COMPONENT_USER_GROUPTRIG				=0x2d,
};
...
typedef struct {
   ...
   uint8_t          backoff;
   bool				LOWERPOWER_RUN;//zyx added if 
} neighborRow_t;
END_PACK
