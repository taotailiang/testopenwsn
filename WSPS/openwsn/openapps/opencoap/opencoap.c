...
void opencoap_receive(OpenQueueEntry_t* msg) {
   ...
   ////index = opencoap_options_parse(&msg->payload[0], msg->length, coap_incomingOptions, &coap_incomingOptionsLen);//解决0x67开头报文的错误
   ...
   option_count =0;//20180713zyx// opencoap_find_option(coap_incomingOptions, coap_incomingOptionsLen, COAP_OPTION_NUM_STATELESSPROXY, &option_index);
   if(option_count >= 1) {
   ...
   //== Proxy Scheme option
   option_count =0;//20180713zyx //opencoap_find_option(coap_incomingOptions, coap_incomingOptionsLen, COAP_OPTION_NUM_PROXYSCHEME, &option_index);
   if(option_count >= 1) {
   ...
   option_count =0;//20180713zyx //opencoap_find_option(coap_incomingOptions, coap_incomingOptionsLen, COAP_OPTION_NUM_OBJECTSECURITY, &option_index);
   if(option_count >= 1) {
   ...
   if (
      ...
      temp_desc->callbackRx(msg,&coap_header,&coap_incomingOptions[0], NULL, NULL);//lcg 20180625 add
...
void opencoap_sendDone(OpenQueueEntry_t* msg, owerror_t error) {
   ...
   {
      if (
      												/* this is added by zyx same group trig 20190220*/
         (temp_resource->componentID==msg->creator||msg->creator==COMPONENT_USER_GROUPTRIG) &&
